...
const secureHostWarning = chalk.redBright(`
    ...
    highly recommends using the ${chalk.whiteBright('"provideSecureHost"')} configuration
...
const PWADevServer = {
  ...
  async configure(config) {
    ...
    const devServerConfig = {
      ...
      contentBase: false,
      // UpwardPlugin serves static files
      compress: true,
      hot: true,
      watchOptions: {
        // polling is CPU intensive - provide the option to turn it on if needed
        poll: !!parseInt(process.env.PWA_STUDIO_HOT_RELOAD_WITH_POLLING) || false
      },
      host: '0.0.0.0',
      port: process.env.PWA_STUDIO_PORTS_DEVELOPMENT || (await portscanner.findAPortNotInUse(10000)),
      stats: {
      ...
      after(app, server) {
        ...
        let readyNotice = chalk.green(`PWADevServer ready at ${chalk.greenBright.underline(devServerConfig.publicPath)}`);

        if (config.graphqlPlayground) {
          readyNotice += '\n' + chalk.blueBright(`GraphQL Playground ready at ${chalk.blueBright.underline(new url.URL('/graphiql', devServerConfig.publicPath))}`);
        }

        server.middleware.waitUntilValid(() => console.log(boxen(readyNotice, {
          borderColor: 'gray',
          float: 'center',
          align: 'center',
          margin: 1,
    ...
    if (id || provideSecureHost) {
      const hostConf = {}; // backwards compatibility

      if (id) {
        const desiredDomain = id + '.' + configureHost.DEV_DOMAIN;
        console.warn(debug.errorMsg(chalk.yellowBright(`
...
be configured to have the same effect as 'id'.
  ...
  To create the subdomain ${desiredDomain}, use:
    ${chalk.whiteBright(`provideSecureHost: { subdomain: "${id}", addUniqueHash: false }`)}

  To omit the default ${configureHost.DEV_DOMAIN} and specify a full alternate domain, use:
    ${chalk.whiteBright(`provideSecureHost: { exactDomain: "${id}.example.dev" }`)}
  ...
  ${helpText}`)));
        ...
        throw new Error(debug.errorMsg(`Unrecognized argument to 'provideSecureHost'. Must be a boolean or an object with 'addUniqueHash', 'subdomain', and/or 'domain' properties.`));
      }
      ...
      const {
        ...
        ports,
        ssl
      } = await configureHost(hostConf);
      devServerConfig.host = hostname;
      devServerConfig.https = ssl; // workaround for https://github.com/webpack/webpack-dev-server/issues/1491

      devServerConfig.https.spdy = {
        protocols: ['http/1.1']
      };
      const requestedPort = process.env.PWA_STUDIO_PORTS_DEVELOPMENT || ports.development;

      if ((await portscanner.checkPortStatus(requestedPort)) === 'closed') {
        devServerConfig.port = requestedPort;
      } else {
        console.warn(chalk.yellowBright('\n' + debug.errorMsg(`This project's dev server is configured to run at ${hostname}:${requestedPort}, but port ${requestedPort} is in use. The dev server will run temporarily on port ${chalk.underline.whiteBright(devServerConfig.port)}; you may see inconsistent ServiceWorker behavior.`) + '\n'));
      }
    } else {
    ...
    if (graphqlPlayground) {
      const {
        queryDirs = []
      } = config.graphqlPlayground;
      const endpoint = '/graphql';
      const queryDirListings = await Promise.all(queryDirs.map(async dir => {
      ...
      const queryDirContents = await Promise.all(queryDirListings.map(({
        dir,
        files
      }) => Promise.all(files.map(async queryFile => {
      ...
      devServerConfig.before = app => {
        oldBefore(app); // this middleware has a bad habit of calling next() when it
    ...
    devServerConfig.publicPath = devServerConfig.public ? `https://${devServerConfig.public}/` : url.format({
...
//# sourceMappingURL=PWADevServer.js.map