...
const toRootComponentMapKey = (type, variant = 'default') => `RootCmp_${type}__${variant}`;
/**
...
class MagentoRootComponentsPlugin {
  ...
  async buildFetchModule() {
    const {
      context,
      rootComponentsDirs
    } = this.opts; // Create a list of absolute paths for root components. When a
    ...
    const rootComponentImporters = await rootComponentsDirsAbs.reduce(async (importersPromise, rootComponentDir) => {
      ...
      await Promise.all(rootComponentFiles.map(async rootComponentFile => {
        const rootComponentSource = await readFile(rootComponentFile, 'utf8');
        const {
          directives = [],
          errors
        ...
        if (rootComponentDirectives.length > 1) {
          console.warn(`Found more than 1 RootComponent Directive in ${rootComponentFile}. Only the first will be used`);
        }
        ...
        if (!pageTypes || pageTypes.length === 0) {
          console.warn(`No pageTypes specified for RootComponent ${rootComponentFile}. RootComponent will never be used.`);
        } else {
          pageTypes.forEach(pageType => {
            const key = toRootComponentMapKey(pageType, variant);
            importerSources[key] = `() => import(/* webpackChunkName: "${key}" */'${relative(context, rootComponentFile)}')`;
          });
        }
      }));
    ...
    this.contents = `
            ...
            ${Object.entries(rootComponentImporters).map(entry => entry.join(':')).join(',\n')}
            };
