...
export const signIn = credentials => async function thunk(...args) {
  const [dispatch] = args;
  ...
  try {
    ...
    await dispatch(actions.signIn.receive(userDetails)); // Now that we're signed in, forget the old (guest) cart
...
export const signOut = ({
  ...
  await clearToken();
  await dispatch(actions.signIn.reset()); // Now that we're signed out, forget the old (customer) cart
...
export const getUserDetails = () => async function thunk(...args) {
...
export const createNewUserRequest = accountInfo => async function thunk(...args) {
  const [dispatch] = args;
...
export const resetPassword = ({
  ...
  const [dispatch] = args;
  dispatch(actions.resetPassword.request(email)); // TODO: actually make the call to the API.
  // For now, just return a resolved promise.

  const response = await Promise.resolve(email);
  dispatch(actions.resetPassword.receive(response));
};
export const completePasswordReset = email => async dispatch => dispatch(actions.completePasswordReset(email));
